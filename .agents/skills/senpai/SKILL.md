...
| `collect --task-id ID --attempt N --from PATH` | Publish framed JSON Result |
...
7. On exit, `collect` the framed JSON Result. Worker `merge` is ignored. Same-attempt republish fails — mint a new attempt to retry.
