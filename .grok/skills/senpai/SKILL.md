...
5. Write the prompt to a file under the control root. Call `launch --prompt-file` — never `-p "$(cat …)"` from a skill, and never `tee … &` as the recorded PID. The helper copies that file and feeds it on the runner's stdin (`claude --print`, `codex exec -`). Do not pass `--prompt-file` to the `claude`/`codex` binaries; they reject it.
